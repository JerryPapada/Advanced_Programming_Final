...
try:
    ...
    print("Spacy model not found. Please run: python -m spacy download el_core_news_sm")
    nlp = None

def process_mati_data(file_path="mati.csv"):
    #----------------------
    ...
    def lemmatize_texts(texts):
        cleaned_texts = []
        # Added check for nlp model
        if nlp:
            for doc in tqdm(nlp.pipe(texts, batch_size=1000, disable=["parser", "ner","tagger"]), total=len(texts), desc="Lemmatization"):
                tokens = [
                    token.text for token in doc
                    if not token.is_stop and token.text not in extra_stopwords and token.is_alpha and len(token.text) >= 4
                ]
    ...
    # REMOVED: df = pd.read_csv("data/mati_preprocessed.csv") 
    ...
    # Return the dataframe so other files can use it
    return df_relevant

# Execution block: This only runs if you run THIS file directly.
# It does NOT run if you import this file.
if __name__ == "__main__":
    df_result = process_mati_data(file_path="mati.csv")
    print(df_result[df_result["is_relevant"]].sample(10)[["text"]])